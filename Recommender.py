...
for i in animeUrl:
    ...
    if page is None:
        if animeUrl.index(i) == 0:
            print("Timed out while accessing the first completed anime, in its current form the program is unable to continue")
...
for i in recDict:
    uprint("{0:70} {1}".format(i, "\tScored: {:,}".format(recDict[i])))
